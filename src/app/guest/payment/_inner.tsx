...
export default function PaymentInner() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-600 to-emerald-700 flex items-center justify-center px-4">
      <div className="max-w-sm w-full text-center">
        <div className="bg-white rounded-3xl p-8 shadow-2xl">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Malipo yamefanikiwa!</h1>
          ...
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl p-5 mb-5">
            <p className="text-xs text-gray-500 mb-2 uppercase tracking-wide font-medium">Namba yako ya vocha</p>
          ...
          <button onClick={copyVoucher}
            ...
            {copied
              ? <><Check className="w-5 h-5" /> Imenakiliwa! Inakurudisha…</>
              : <><Copy className="w-5 h-5" /> Nakili namba</>}
          </button>
          ...
          {(portalUrl || redirectUrl) && !copied && (
            <button onClick={() => window.location.href = portalUrl || decodeURIComponent(redirectUrl)}
              ...
              Rudi kwenye kuingia Wi-Fi <ArrowRight className="w-4 h-4" />
            </button>
          )}

          <p className="text-gray-400 text-xs mt-4">Weka namba hii kwenye ukurasa wa kuingia Wi-Fi</p>
        </div>
        <p className="text-green-200 text-xs mt-4">KASI Wi-Fi</p>
      </div>
    </div>
  )
        ...
        <div className="max-w-sm w-full text-center">
          <div className="bg-white rounded-3xl p-8 shadow-2xl">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">
              {status === 'PAYMENT_CANCELLED' ? 'Malipo yameghairiwa' :
              status === 'PAYMENT_TIMEOUT'   ? 'Muda wa malipo umeisha' : 'Malipo yameshindikana'}
            </h1>
            <p className="text-gray-500 mb-6">{message || 'Tafadhali jaribu tena.'}</p>
  ...
  const statusLabel = status === 'OMADA_AUTHORIZING' || status === 'PAYMENT_SUCCESS'
    ? 'Inaandaa vocha yako…'
    : `Inasubiri malipo${'.' .repeat(dots + 1)}`
      ...
      <div className="max-w-sm w-full">
        <div className="bg-white rounded-3xl p-8 shadow-2xl text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-2">
            {status === 'OMADA_AUTHORIZING' || status === 'PAYMENT_SUCCESS' ? 'Malipo yamethibitishwa!' : 'Angalia simu yako'}
          </h1>
          ...
          {status !== 'OMADA_AUTHORIZING' && status !== 'PAYMENT_SUCCESS' && (
            <>
              ...
              <p className="text-gray-500 mb-4">Ombi la malipo limetumwa kwenye:</p>
              ...
              <p className="text-gray-500 text-sm mb-6">Weka <strong>PIN ya pesa ya simu</strong></p>
            </>
          )}

          {(status === 'OMADA_AUTHORIZING' || status === 'PAYMENT_SUCCESS') && (
            <p className="text-gray-500 text-sm mb-6">Inatengeneza namba yako ya vocha…</p>
          )}
          ...
          <div className="bg-gray-50 rounded-xl p-3 text-left">
            <p className="text-gray-400 text-xs">Namba ya kumbukumbu: <span className="font-mono">{reference}</span></p>
          </div>

          <button onClick={poll} className="mt-4 text-brand-500 text-sm flex items-center gap-1 mx-auto hover:text-brand-700">
            <RefreshCw className="w-3.5 h-3.5" /> Angalia hali
          </button>

          {waitSeconds > 180 && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-gray-400 text-xs mb-2">Imechukua muda mrefu?</p>
              <button onClick={() => {
                const token = new URLSearchParams(window.location.search).get('token')
        ...
        <p className="text-brand-300 text-xs text-center mt-4">Malipo salama kupitia MalipoPay · KASI Wi-Fi</p>
      </div>
    </div>
  )
}
